...
if($username=='' || $email==''){
	...
	if($isemail > 0){
		...
		//如果Email存在就跳转到验证密码页面
		header("Location: ".SITE_URL."index.php?app=pubs&ac=plugin&plugin=sina&in=email&email=".$email);
		...
		$msg_content = '亲爱的 '.$username.' ：<br />您成功加入了 '
									.$TS_SITE['base']['site_title'].'<br />在遵守本站的规定的同时，享受您的愉快之旅吧!<br />你除了能用新浪微博账户登录外，还可以用Email登录本站<br />登录Email：'.$email.'<br />登录密码：'.$pwd;
