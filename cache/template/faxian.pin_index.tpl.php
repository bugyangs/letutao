			...
			<div class="tag-book s1024">
                ...
                <?php foreach((array)$catetags as $key=>$item) {?>
                       ...
                       <?php foreach((array)$item['subcate'] as $kkey=>$iitem) {?>
						<div class="top">
							<div class="tag-img">
								<a href="<?php echo SITE_URL;?><?php echo tsurl($item['appname'])?>" title="<?php echo $item['cate_name'];?>">
                            ...
                            <li><a class="<?php if($tag == $iiitem) { ?>cur<?php } ?>" href="<?php echo SITE_URL;?><?php echo tsurl($cate_appname,'',array(tag=>$iiitem))?>"><?php echo $iiitem;?></a></li>
							<?php }?>
							 <?php } ?> 																	</ul>
						</div>
  <?php }?>
...
<?php } ?>
			<div class="goods-title clearfix" id="J_GoodsTitle">
													<h2><?php if($tag) { ?><?php echo $tag;?><?php } elseif ($kw) { ?>搜索"<b><?php echo $kw;?></b>"的结果<?php } else { ?>全部<?php } ?></h2>	
								<div class="sort">
                    <a <?php if($sort=='count_like') { ?>class="on"<?php } ?> href="<?php echo SITE_URL;?><?php echo tsurl($cate_appname,'','',$tag,'count_like',$kw)?>">受欢迎</a>	
<a <?php if($sort=='count_worth') { ?>class="on"<?php } ?> href="<?php echo SITE_URL;?><?php echo tsurl($cate_appname,'','',$tag,'count_worth',$kw)?>">值得买</a>
<a <?php if($sort=='count_view') { ?>class="on"<?php } ?> href="<?php echo SITE_URL;?><?php echo tsurl($cate_appname,'','',$tag,'count_view',$kw)?>">热门</a>
<a <?php if($sort=='count_comment') { ?>class="on"<?php } ?> href="<?php echo SITE_URL;?><?php echo tsurl($cate_appname,'','',$tag,'count_comment',$kw)?>">热评</a>

				</div>
                                <div class="range-box">
                         <a <?php if($sort=='0~100') { ?>class="on"<?php } ?> href="<?php echo SITE_URL;?><?php echo tsurl($cate_appname,'','',$tag,'0~100')?>">0-100元</a>|
<a <?php if($sort=='100~200') { ?>class="on"<?php } ?> href="<?php echo SITE_URL;?><?php echo tsurl($cate_appname,'','',$tag,'100~200',$kw)?>">100-200元</a>|
<a <?php if($sort=='200~500') { ?>class="on"<?php } ?> href="<?php echo SITE_URL;?><?php echo tsurl($cate_appname,'','',$tag,'200~500',$kw)?>">200-500元</a>|

<a <?php if($sort=='500~9999999') { ?>class="on"<?php } ?> href="<?php echo SITE_URL;?><?php echo tsurl($cate_appname,'','',$tag,'500~9999999',$kw)?>">500元以上</a>
                </div>
                 <div class="tabpin">			
                      <ul class="pin-board-switcher clearfix">
       <li class="first selected"><a href="<?php echo SITE_URL;?><?php echo tsurl('faxian','',array('subcate'=>$subcate,'k'=>$kw,'tag'=>$tag,'sort'=>$sort,'style'=>'pin'))?>">瀑布流</a></li>
        <li class="last"><a href="<?php echo SITE_URL;?><?php echo tsurl('faxian','',array('subcate'=>$subcate,'k'=>$kw,'tag'=>$tag,'sort'=>$sort,'style'=>'boards'))?>">画板</a></li>
    </ul>	
    </div>				
			</div>
...
if(goodsPage.sumGoodsNum > 0){
	jQuery(function() {
		...
		jQuery.guang.goods.conf.ajaxData.tagId = "<?php echo $tagid;?>";
