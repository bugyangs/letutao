...
switch($ts){
	...
	case "do":
		...
		if((!isset($email)) || !(isset($pwd)) || (!isset($repwd)) || (!isset($resetpwd))){
			qiMsg("迷路了吗？");
			...
			$db->query("update ".dbprefix."user set pwd='".md5($salt.$pwd)."' and salt='$salt' where email='$email'");
			
			qiMsg("密码修改成功^_^","点击登陆","index.php");
		}
