...
$p_face = $_POST['face'];
$str_info =  _get('str_info','base64_decode','');

if($str_info){
	
	$str_info = json_decode($str_info,true);
		$userData['userid'] = 999999;
		$userData['username'] = $str_info['nickname'];
		$userData['figureurl_qq_1'] = $str_info['figureurl_qq_1'];
		$_SESSION['tsuser']	= $userData;
		
		echo '<script>if(window.opener){window.opener.followGuang(100,"","","false",true);}window.close();	</script>';
		exit;
}
...
if($username=='' || $email==''){
	...
	*/
			$db->query("update ".dbprefix."user_info set `net_face`='$p_face' where `userid`='$userid'");
		}
